...
for(net in networks) {
  ...
  write.csv(preds,paste0(rdir,"/results/lesion/exclusion/predictions_network",net,".csv"),row.names=F)
...
write.csv(performance_metrics,paste0(rdir,"/results/lesion/exclusion/performance_metrics.csv"),row.names=F)
...
write.csv(fold_info,paste0(rdir,"/results/lesion/exclusion/fold_info.csv"),row.names=F)
